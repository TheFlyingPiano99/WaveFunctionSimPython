...
def sim():
    ...
    initial_wp_velocity_bohr_radii_hartree_per_h_bar = np.array(config['initial_wp_velocity_bohr_radii_hartree_per_h_bar'])
    velocity_magnitude = np.dot(initial_wp_velocity_bohr_radii_hartree_per_h_bar, initial_wp_velocity_bohr_radii_hartree_per_h_bar)**0.5
    ...
    initial_wp_momentum_h_per_bohr_radius = math_utils.classical_momentum(mass=particle_mass, velocity=initial_wp_velocity_bohr_radii_hartree_per_h_bar)
    momentum_magnitude = np.dot(initial_wp_momentum_h_per_bohr_radius, initial_wp_momentum_h_per_bohr_radius)**0.5
    print(f"Initial mean momentum of particle is {momentum_magnitude} h-bar / Bohr radius")
    ...
    initial_wp_position_bohr_radii_3 = np.array(config["initial_wp_position_bohr_radii_3"])
    ...
    wave_tensor = wave_packet.init_gaussian_wave_packet(N=N, delta_x_bohr_radii=delta_x_bohr_radii, a=a, r_0_bohr_radii_3=initial_wp_position_bohr_radii_3,
    ...
    try:
        ...
        only_the_obstacle_potential = np.load(file='cache/only_the_obstacle_potential.npy')
    except OSError:
    ...
    print(f"Time spent with initialisation: {time.time() - initialisation_start_time_s} s.")
    print("***************************************************************************************")
    print("Starting simulation")
    canvas = volume_visualization.VolumeCanvas(volume_data=probability_density, secondary_data=only_the_obstacle_potential)
    ...
    measurement_volume_full = measurement.AAMeasurementVolume(bottom_corner=(0,0,0), top_corner=(N,N,N), label="Full volume")
    measurement_volume_first_half = measurement.AAMeasurementVolume(bottom_corner=(0,0,0), top_corner=(N,N,int(N/2)), label="First half")
    measurement_volume_second_half = measurement.AAMeasurementVolume(bottom_corner=(0,0,int(N/2)), top_corner=(N,N,N), label="Second half")
    ...
    for i in range(1001):
        ...
        measurement_volume_second_half.integrate_probability_density(probability_density)

        print(f"Integral of probability density P = {measurement_volume_full.get_probability()}.")
        print(f"Probability of particle in first half P(first) = {measurement_volume_first_half.get_probability():.3}.")
        print(f"Probability of particle in second half P(second) = {measurement_volume_second_half.get_probability():.3}.")

        canvas.update(probability_density, iter_count=i, delta_time_h_bar_per_hartree=delta_time_h_bar_per_hartree)
