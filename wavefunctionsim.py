...
def init_potential_box(N, wall_thickness, potential_wall_hight):
    V = np.zeros(shape=(N, N, N), dtype=float)
    for x in range(0, N):
        for y in range(0, N):
            for z in range(0, N):
                r = np.array([x, y, z]) / np.array([N, N, N])
                ...
                if abs(r[0] - 1.0) < wall_thickness:
                    V[x, y, z] = potential_wall_hight * (wall_thickness - abs(r[0] - 1.0)) / wall_thickness
                # Y-axis:
                ...
                if abs(r[1] - 1.0) < wall_thickness:
                    V[x, y, z] = potential_wall_hight * (wall_thickness - abs(r[1] - 1.0)) / wall_thickness
                # Z-axis:
                ...
                if abs(r[2] - 1.0) < wall_thickness:
                    V[x, y, z] = potential_wall_hight * (wall_thickness - abs(r[2] - 1.0)) / wall_thickness
    return V
    ...
    for x in range(0, N):
        for y in range(0, N):
            for z in range(0, N):
                ...
                wave_tensor[x, y, z] = (2.0 / math.pi / a**2)**(3.0/4.0) * exp_i(np.dot(k_0, r)**0.5) * math.exp(- np.dot(r - r_0, r - r_0) / a**2)
    return wave_tensor
...
packet_width = 1.0
a = packet_width * 2.0
wave_tensor = init_gaussian_wave_packet(N=N, delta_x=delta_x, a=a, r_0=np.array([8, 8, 8]), k_0=np.array([0, 1.5, 0]))
...
V = init_potential_box(N, wall_thickness=0.1, potential_wall_hight=10000.0)
